...
class LineSearchMinimizationWithGradientDescent:
    ...
    """
    def __init__(self, f, x0, obj_tol=10**-12, param_tol=10**-8, max_iter=100):
        self.f = f
        self.x0 = x0
    ...
    def minimize(self):
        ...
        for i in range(self.max_iter):
            ...
            # Check for convergence,  if curr_gradient is a zero vector it indicates that we have found a stationary point, which is a candidate for being a local minimum
            if np.abs(next_val - curr_val) < self.obj_tol or np.linalg.norm(next_x - x) < self.param_tol or not curr_gradient.any():
                print(f"Convergence achieved after {i+1} iterations.")
                self.iterations.append(next_x.copy())
                self.objective_values.append(next_val)
                return next_x, next_val, success_status
            
            # Update variables for the next iteration
            x, curr_val, curr_gradient = next_x, next_val, next_gradient
    
        print("Maximum iterations reached without convergence.")
        return x, curr_val, success_status
